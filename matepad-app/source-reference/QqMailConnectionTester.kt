...
class QqMailConnectionTester {
    fun testConnection(email: String, authCode: String): MailConnectionResult {
        val normalizedEmail = email.trim()

        if (normalizedEmail.isBlank() || authCode.isBlank()) {
            return MailConnectionResult(false, "邮箱或授权码为空")
        }
        ...
        return try {
            ...
            MailConnectionResult(false, "邮箱授权码错误或 IMAP 未开启")
...
data class MailConnectionResult(
    val success: Boolean,
    val message: String
)
