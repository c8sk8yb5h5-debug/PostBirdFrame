...
const thumbs: PhotoItem[] = thumbLabels.map((label, index) => ({
  id: index + 1,
  label,
  year: 2026,
  month: 1,
...
function ScenePhoto({ item }: { item: PhotoItem }) {
  return (
    <div className="relative h-full w-full overflow-hidden bg-[#101010]">
      ...
      <div className="absolute left-[7%] top-[8%] rounded-full bg-black/45 px-4 py-2 text-sm text-white backdrop-blur-md">
        {item.label}
      </div>
    </div>
  );
...
function Thumb({ item, active, onClick }: ThumbProps) {
  return (
    <motion.button
      layout
      onClick={onClick}
      ...
      className={`relative aspect-square w-full overflow-hidden rounded-2xl border transition ${
        active ? "border-[#78d6ff] ring-2 ring-[#78d6ff]/50" : "border-white/10"
      }`}
      style={{
        background: `linear-gradient(135deg, hsl(${item.hue}, 65%, 70%), hsl(${
          (item.hue + 60) % 360
        }, 55%, 42%))`,
      }}
    >
      ...
      <div className="absolute bottom-2 left-2 right-2 rounded-lg bg-black/30 px-2 py-1 text-left text-[10px] text-white backdrop-blur-sm">
        {item.label}
      </div>
    </motion.button>
  );
...
function CircleControls({ item, onOpenSettings }: CircleControlsProps) {
  return (
    <div className="absolute bottom-5 left-5 z-30 flex items-end gap-3">
      <motion.button
        whileTap={{ scale: 0.92 }}
        className="flex h-16 w-16 flex-col items-center justify-center rounded-full border border-white/35 bg-white/12 text-white shadow-[0_12px_35px_rgba(0,0,0,.35)] backdrop-blur-xl"
      >
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        className="flex h-16 w-16 items-center justify-center rounded-full border border-white/35 bg-white/12 text-white shadow-[0_12px_35px_rgba(0,0,0,.35)] backdrop-blur-xl"
      >
        <Settings size={26} />
      </motion.button>
    </div>
...
function YearPicker({ open, currentYear, onClose, onConfirm }: YearPickerProps) {
  const [year, setYear] = useState(currentYear);
  const touchStartY = useRef<number | null>(null);

  const inc = (v: number) => setYear((y) => Math.max(2000, Math.min(2035, y + v)));
        ...
        >
          <motion.div
            ...
            onTouchEnd={(e) => {
              ...
              if (Math.abs(dy) > 20) inc(dy < 0 ? 1 : -1);
          ...
          >
            ...
            <div className="flex items-center justify-center gap-4 py-5">
              <button onClick={() => inc(-1)} className="rounded-full bg-white/10 p-2">
                <ChevronUp />
              </button>
              <div className="flex w-28 flex-col items-center gap-3">
                <span className="text-xl text-white/40">{year - 1}</span>
                <span className="rounded-2xl bg-white/18 px-6 py-3 text-4xl font-semibold">
                  {year}
                </span>
                <span className="text-xl text-white/40">{year + 1}</span>
              </div>
            ...
            <button
              ...
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-white px-4 py-3 text-sm font-semibold text-[#111]"
            >
              <Check size={17} /> 应用年份
            </button>
          </motion.div>
      ...
      <div className="space-y-4 overflow-y-auto px-5 py-4">
        <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-4">
          <div className="mb-3 flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm font-medium">
              <Mail size={16} /> QQ 邮箱接收
            </div>
            <span className="rounded-full bg-emerald-400/18 px-2 py-1 text-[10px] text-emerald-200">
              已连接
            </span>
          </div>
          ...
          <div className="mt-3 flex gap-2">
            <button
              onClick={() => setStatus("正在检查邮箱...")}
              className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-white/12 px-3 py-2 text-xs"
            >
              <RefreshCw size={14} /> 立即检查
            </button>
            <button
              onClick={() => setStatus("已准备更新包")}
              className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-white/12 px-3 py-2 text-xs"
            >
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <button onClick={onOpenYearPicker} className="rounded-2xl bg-white/12 px-3 py-3">
              年份 {yearFrom}
            </button>
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {thumbs.slice(0, 12).map((t) => (
              <Thumb key={t.id} item={t} active={selectedId === t.id} onClick={() => setSelectedId(t.id)} />
            ))}
          </div>
        </div>
      </div>
...
export default function MatePadPreviewPrototype() {
